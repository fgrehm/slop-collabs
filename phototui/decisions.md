...
- **Real albums are full-resolution; decode them once into a thumbnail cache.** OpenTUI's `NativeImage.decode` always decodes full-resolution (no scaled-decode option), so a 12MP photo costs a ~48MB allocation per decode even though we only draw a ~30px cell. The worker keeps a disk thumbnail cache: first visit decodes the original and writes a fixed 256px raw-RGBA thumbnail (custom `PTRGBA` header + pixels) under `/tmp/phototui-thumbs` (POC location; a real cache would live under XDG_CACHE_HOME and be invalidated when the source changes); later visits read the tiny thumbnail file and downscale from that. Measured: 20 images first pass 556ms (decode+write), second pass 8ms (read thumbs) - ~70x. The first pass is off-thread so the UI stays responsive; repeat browsing is instant.
- **The frame rate was capped at 30fps by our own config, not by OpenTUI or the terminal.** `createCliRenderer({ targetFps: 30 })` sets `targetFrameTime = 1000/30 = 33.3ms`, and the render loop schedules the next frame at `targetFrameTime` after each frame. So the app was running at a hard 30fps cap we chose. Raising `targetFps` to 60 (leaving `maxFps` at its 60 default) doubled the frame rate with no dropped frames in a pty: avg dt 34.1ms -> 17.0ms, max dt 43.1ms -> 29.6ms. `maxFps` is NOT a hard cap on the steady-state loop - it only bounds `immediateRerenderRequested` (intermediate) renders; `targetFps` is the real pacing knob. `targetFps: 120` ran ~108fps but showed frame drops (max dt 46.5ms), so 60 is the sweet spot for now. The `OTUI_FRAME_LOG=1` env var starts frame logging on so frame-rate regressions can be measured headlessly without pressing `f`.
